...
interface StateCodePairs {
  [key: string]: string[];
}

const stateCodePairs: StateCodePairs = {
  JHR: ['01', '21', '22', '24'],
  KDH: ['02', '25', '26', '27'],
  KTN: ['03', '28', '29'],
  MLK: ['04', '30'],
...
interface CountryCodePairs {
  [key: number]: { country: string; region: string | null };
}

const countryCodePairs: CountryCodePairs = {
...
function parseForeign(code: number) {
  return Object.assign({ state: null }, countryCodePairs[code]);
}

export function parseBirthplace(code: number): any {
  const transformedCode =
    code.toString().length === 1 ? `0${code}` : code.toString();
  if (isMalaysia(code)) return parseMalaysia(transformedCode);
  if (isForeign(code)) return parseForeign(code);

  return null;
}

export function isValidBirthplace(code: number): boolean {
